...
app.put("/actividades/:id", async (req, res) => {
  ...
  try {
    ...
    const conflicts = await db.query(checkQuery, [
      id,
      Fecha_Inicio,
      Fecha_Fin,
      Fecha_Inicio,
    ...
    if (conflicts.length > 0) {
      let conflictMessages = conflicts.map((conflict) => {
        let conflictDate = "";
        if (
          Fecha_Inicio >= conflict.Fecha_Inicio &&
          Fecha_Inicio <= conflict.Fecha_Fin
        ) {
          conflictDate = `Inicio: ${Fecha_Inicio}`;
        }
        if (
          Fecha_Fin >= conflict.Fecha_Inicio &&
          Fecha_Fin <= conflict.Fecha_Fin
        ) {
    ...
    await db.query(
      "UPDATE tbActividad SET Titulo_Actividad = ?, Fecha_Inicio = ?, Fecha_Fin = ? WHERE Id_Actividad = ?",
...
app.put("/etapas/:id", async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE tbEtapa SET fecha_inicio = ?, fecha_fin = ? WHERE id_etapa = ?",
...
app.get("/proyectos", (req, res) => {
  ...
  db.query(query, (err, results) => {
    ...
    const proyectos = {
      tercerCiclo: results.filter(
        (p) => p.Id_Nivel && p.Id_Nivel >= 1 && p.Id_Nivel <= 3
      ),
      bachillerato: results.filter(
        (p) => p.Id_Nivel && p.Id_Nivel >= 4 && p.Id_Nivel <= 6
      ),
    };
...
app.get("/seccionGrupo", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error obteniendo las secciones y grupos" });
      return;
    }
    res.json(results);
  });
...
app.get("/proyectosId", (req, res) => {
  ...
  if (!prefijo) {
    return res
      .status(400)
      .json({ error: "Se requiere un prefijo para la búsqueda de proyectos" });
  }
...
app.get("/estudiantes/nivel/:nivelId", (req, res) => {
  ...
  db.query(query, [nivelId], (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error obteniendo los estudiantes por nivel" });
      return;
    }
    res.json(results);
  });
...
app.get("/estudiantes/disponibles", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error obteniendo los estudiantes disponibles" });
      return;
    }
    res.json(results);
  });
...
app.post("/proyectos/asignar-estudiantes", (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        error: "Se requiere un ID de proyecto y al menos un ID de estudiante",
      });
  }
  ...
  db.query(query, params, (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error asignando estudiantes al proyecto" });
      return;
    }
...
app.post("/creacionProyectos", async (req, res) => {
  ...
  if (!estudiantesIds || !Array.isArray(estudiantesIds)) {
    return res
      .status(400)
      .json({ error: "El formato de estudiantesIds es incorrecto" });
  }
  ...
  try {
    ...
    await db.query(insertProyectoQuery, [
      idProyecto,
      nombre,
      nivelId,
      seccionId,
    ...
    console.log(
      `Actualizaciones completadas: ${updateResult.affectedRows}/${estudiantesIds.length}`
    );
...
app.post("/crearRubrica", async (req, res) => {
  ...
  if (!criterios || !Array.isArray(criterios)) {
    return res
      .status(400)
      .json({ error: "El formato de criterios es incorrecto" });
  }
  ...
  try {
    ...
    const result = await db.query(insertRubricaQuery, [
      nombreRubrica,
      tipoEvaluacionId,
    ]);
    ...
    const criteriosParams = criterios.flatMap((criterio) => [
      idRubrica,
    ...
    console.log(
      `Criterios insertados: ${insertResult.affectedRows}/${criterios.length}`
    );
    ...
    db.rollback(() => {
      ...
      res
        .status(500)
        .json({ error: "Error al crear la rúbrica: " + error.message });
    });
  } finally {
    db.close();
  }
...
app.get("/proyectos/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error obteniendo el proyecto", details: err.message });
  } finally {
    db.close();
  }
});
...
app.get("/proyectos/:id/estudiantes", (req, res) => {
  ...
  db.query(query, [idProyecto], (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error obteniendo los estudiantes del proyecto" });
      return;
    }
...
app.put("/actualizarProyecto", async (req, res) => {
  ...
  try {
    ...
    if (projectIdToUpdate !== idProyecto) {
      console.log(
        `ID del proyecto cambió: ${projectIdToUpdate} -> ${idProyecto}. Desvinculando estudiantes primero.`
      );
      ...
      await db.query(updateProyectoQuery, [
        nombre,
        valorGoogle,
        nivelId,
        seccionId,
      ...
      console.log(
        "Proyecto actualizado correctamente, ID proyecto:",
        idProyecto
      );
    ...
    if (estudiantesIds.length > 0) {
      ...
      console.log(
        `Actualizaciones de estudiantes completadas: ${updateResult.affectedRows}/${estudiantesIds.length}`
      );
      ...
      console.log(
        "Proyecto actualizado sin estudiantes - Transacción completada"
      );
    ...
    db.rollback(() => {
      ...
      res
        .status(500)
        .json({ error: "Error al actualizar el proyecto: " + error.message });
    });
  } finally {
    db.close();
  }
...
app.put("/api/usuarios/:id", async (req, res) => {
  ...
  if (!nombre || !apellido || !correo || !idRol) {
    return res
      .status(400)
      .json({ message: "Nombre, apellido, correo y rol son obligatorios" });
  }
...
app.get("/api/seccion-grupos", async (req, res) => {
  ...
  try {
    const query =
      "SELECT Id_SeccionGrupo AS id, Nombre_SeccionGrupo AS nombre FROM tbSeccionGrupo";
...
app.post("/api/criterios", async (req, res) => {
  ...
  if (!id_Rubrica || !nombre_Criterio) {
    return res
      .status(400)
      .json({ message: "id_Rubrica y nombre_Criterio son obligatorios" });
  }
...
app.get("/api/proyectos/grado", async (req, res) => {
  const db = new DBConnection();
  const { idNivel, idEspecialidad } = req.query;
  ...
  // Filtro por especialidad o proyectos aplicables a todas
  if (idEspecialidad) {
    query += " AND (Id_Especialidad = ? OR Id_Especialidad IS NULL)";
    values.push(idEspecialidad);
  }

  try {
    const proyectos = await db.query(query, values);
    res.json(proyectos);
  } catch (error) {
...
app.get("/api/estudiantes", async (req, res) => {
  ...
  if (search) {
    query +=
      " AND (e.nombre_Estudiante LIKE ? OR e.apellido_Estudiante LIKE ?)";
...
app.get("/api/secciones", async (req, res) => {
  ...
  try {
    const secciones = await db.query(
      "SELECT * FROM tbSeccionGrupo ORDER BY Nombre_SeccionGrupo"
    );
    res.json(secciones);
  } catch (error) {
...
app.get("/api/especialidades", async (req, res) => {
  ...
  try {
    const especialidades = await db.query(
      "SELECT * FROM tbEspecialidad ORDER BY Nombre_Especialidad"
    );
  ...
  try {
    ...
    if (Id_Nivel >= 4 && Id_Nivel <= 6 && !Id_Especialidad) {
      return res
        .status(400)
        .json({
          message: "Especialidad requerida para estudiantes de bachillerato",
        });
    }
...
app.post("/api/guardarEstudiantes", async (req, res) => {
  ...
  try {
    const { codigo, nombres, apellidos, idNivel, idSeccion, idEspecialidad } =
      req.body;
  ...
  async (req, res) => {
    ...
    try {
      ...
      const niveles = await db.query(
        "SELECT Id_Nivel, Nombre_Nivel FROM tbNivel"
      );
      const nivelesMap = new Map(
        niveles.map((n) => [n.Nombre_Nivel.toLowerCase(), n.Id_Nivel])
      );

      const secciones = await db.query(
        "SELECT Id_SeccionGrupo, Nombre_SeccionGrupo FROM tbSeccionGrupo"
      );
      const seccionesMap = new Map(
        secciones.map((s) => [
          s.Nombre_SeccionGrupo.toLowerCase(),
          s.Id_SeccionGrupo,
        ])
      );

      const especialidades = await db.query(
        "SELECT Id_Especialidad, Nombre_Especialidad FROM tbEspecialidad"
      );
      const especialidadesMap = new Map(
        especialidades.map((e) => [
          e.Nombre_Especialidad.toLowerCase(),
          e.Id_Especialidad,
        ])
      );
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (row.Especialidad) {
          ...
          if (!idEspecialidad) {
            errores.push(
              `Fila ${i + 2}: Especialidad '${row.Especialidad}' no encontrada`
            );
            continue;
          }
        }
...
app.post(
  "/api/limpiar-usuarios-inactivos",
  verificarToken,
  async (req, res) => {
    const db = new DBConnection();
    try {
  ...
  try {
    if (req.usuario.idRol !== 1) {
      return res
        ...
        .json({
          message:
            "Acceso denegado. Solo administradores pueden ver el historial.",
        });
    }
...
app.post(
  "/api/resetear-historial-conexiones",
  verificarToken,
  async (req, res) => {
    const db = new DBConnection();
    try {
      ...
      if (req.body.confirmar !== "RESETEAR_HISTORIAL") {
        return res.status(400).json({
          message:
            'Para confirmar, envía el parámetro "confirmar" con valor "RESETEAR_HISTORIAL"',
        });
      }
...
app.delete("/api/eliminar-todos-estudiantes-proyectos", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Eliminados ${estudiantesResult.affectedRows} estudiantes y ${proyectosResult.affectedRows} proyectos`
    );

    res.status(200).json({
      message:
        "Todos los estudiantes y proyectos han sido eliminados correctamente",
